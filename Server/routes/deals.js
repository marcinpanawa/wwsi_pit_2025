...
router.get('/:id', async (req, res) => {
    try {
        ...
        const comments = await Comment.find({ deal: deal._id }).populate('user', 'name').sort({ date: 1 });
        res.render('deals/show', { title: deal.title, deal, comments, layout: './Layouts/layout' });
    } catch (err) {
        console.error(err);
        res.status(500).send('Server Error');
    }
